...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white shadow-lg rounded-xl overflow-hidden">
        <div class="md:flex">
            <!-- ส่วนแสดงรูปภาพ -->
            <div class="md:w-1/2">
                <div class="relative h-[700px] w-full"> <!-- กำหนดความสูงตายตัว -->
                    <img id="main-image" 
                         src="{{ $costume->primaryImage ? Storage::url($costume->primaryImage->image_path) : asset('images/placeholder.jpg') }}" 
                         alt="{{ $costume->name }}" 
                         class="w-full h-full object-cover object-center">
                    
                    @if($costume->stock < 5 && $costume->stock > 0)
                    <div class="absolute top-4 right-4 bg-yellow-500 text-white px-3 py-1 rounded-full text-sm font-bold">
                        เหลือเพียง {{ $costume->stock }} ชุด
                    </div>
                ...
                <!-- แสดงรูปภาพเพิ่มเติม (ถ้ามี) -->
                @if($costume->images->count() > 1)
                <div class="grid grid-cols-4 gap-2 p-4">
                    @foreach($costume->images as $image)
                        <div class="border rounded overflow-hidden cursor-pointer hover:opacity-75 transition duration-200 h-20 w-full" 
                ...
                <div class="mb-6">
                    ...
                    <div class="flex flex-wrap gap-4 mb-4">
                        <div class="bg-gray-100 rounded-lg px-4 py-2">
                            ...
                            <p class="font-medium text-gray-800">{{ $costume->stock > 0 ? 'พร้อมเช่า' : 'ไม่พร้อมเช่า' }}</p>
                        </div>
                        <div class="bg-gray-100 rounded-lg px-4 py-2">
                            <span class="text-gray-600 text-sm">คงเหลือ</span>
                            <p class="font-medium text-gray-800">{{ $costume->stock }} ชุด</p>
                        </div>
                        <div class="bg-gray-100 rounded-lg px-4 py-2">
                    ...
                    <div>
                        ...
                        <select name="color_id" id="color_id" class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-pink-500 focus:border-pink-500" required>
                            ...
                            @foreach($costume->colors as $color)
                                @php
                                    $activeRentalsForColor = $costume->rentals->where('color_id', $color->id)->sum('quantity');
                    ...
                    <div>
                        ...
                        <select name="size_id" id="size_id" class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-pink-500 focus:border-pink-500" required>
                            ...
                            @foreach($costume->sizes as $size)
                                @php
                                    $activeRentalsForSize = $costume->rentals->where('size_id', $size->id)->sum('quantity');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ฟังก์ชันสำหรับเปลี่ยนรูปภาพหลัก
    ...
    function updateQuantityOptions() {
        ...
        const maxStock = Math.min(colorStock, sizeStock, {{ $costume->stock }});
